...
console.log("\n── نزولُ الورقة على الطاولة بلا قفزة ──");
{
  ...
  /* والمنظورُ مُعرَّفٌ على ‎#world‎ ومركزُه ٥٠٪/٤٠٪، فالخانةُ القريبة تكبر
     والبعيدةُ تنحسر — والورقةُ الطائرة تحمل منظورَها حول مركزها هي. قسنا
     الفرق: ‎+١٢٪‎ يسارًا و‎−٧٪‎ أعلى. فيُقاس ويُصحَّح لا يُهمَل. */
  ok(/function slotFit\(el\)/.test(AR), "ثمّة قياسٌ لتصحيح المنظور لكلّ خانة");
  ok(/function landing\(el\)/.test(AR), "ومستطيلُ هبوطٍ مبنيٌّ عليه");
  ok(/fitCache/.test(AR) && /fitCache\.clear\(\)/.test(AR), "ونتيجتُه مخزَّنةٌ وتُمسح عند تغيّر المقاس");
  const fs2 = (AR.match(/function fitStage[\s\S]*?\n}/) || [""])[0];
  ok(/fitCache\.clear\(\)/.test(fs2), "والمسحُ داخل ضبط المسرح نفسه");

  const fp = (AR.match(/async function flyPlay[\s\S]*?\n}/) || [""])[0];
  ok(/landing\(slot\)/.test(fp), "ولعبُ الورقة يهبط بالمقاس المصحَّح", fp.slice(0, 60));
  ...
  /* ولا تتعايشُ ورقتان لحظةً واحدة. جرّبنا الذوبانَ مرّتين:
     ١) ذوبانُ الاثنتين ⇒ المرئيُّ ٧٥٪ في المنتصف ⇒ وميض.
     ٢) ذوبانُ الطائرة وحدَها ⇒ تبقى فوق المستقرّة ٩٠ ملّي ثانية وبينهما
        فرقٌ صغير ⇒ تُرى **ورقتان** لا واحدة.
     فلمّا تطابقت الوضعيّتان لم يبقَ ما يُخفى: استبدالٌ في إطارٍ واحد. */
  ok(!/hold/.test(fp) && !/handoff/.test(fp), "ولا تبقى الطائرةُ فوق المستقرّة — ورقةٌ واحدةٌ دائمًا");
  ok(!/function handoff/.test(AR), "ولا أثرَ للتسليم بالذوبان");
  ok(/f\.remove\(\); res\(\);/.test(AR), "الطائرةُ تُحذَف فورَ الوصول");
}
